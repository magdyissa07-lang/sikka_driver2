...
class _WalletScreenState extends State<WalletScreen> {
    ...
    final amount = await showModalBottomSheet<String>(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            const Text('طلب سحب رصيد', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
            const SizedBox(height: 6),
            const Text('السحب بيتم أسبوعيًا كل يوم أحد إلى حسابك البنكي المسجل.',
                style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
            const SizedBox(height: 12),
            TextField(controller: ctrl, keyboardType: TextInputType.number, decoration: const InputDecoration(hintText: 'المبلغ')),
            const SizedBox(height: 14),
            ElevatedButton(onPressed: () => Navigator.pop(context, ctrl.text), child: const Text('تأكيد طلب السحب')),
          ],
        ),
      ),
    );
    if (amount != null && double.tryParse(amount) != null) {
      final res = await WalletService.withdraw(double.parse(amount));
      if (res['error'] != null) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('المحفظة والمستحقات')),
      body: _loading
              ...
              children: [
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text('الرصيد المستحق', style: TextStyle(color: AppColors.onAccent, fontSize: 13)),
                      const SizedBox(height: 6),
